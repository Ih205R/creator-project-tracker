...
export default function DashboardUpgradePage() {
  ...
  const handleSubscribe = async (plan) => {
    if (!user) {
      router.push(`/login?redirect=/dashboard/upgrade&plan=${plan.id}`);
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/stripe/create-checkout-session`, {
        ...
        body: JSON.stringify({
          ...
          cancelUrl: `${window.location.origin}/subscription/error?type=canceled`,
        }),
      });
            ...
            return (
              ...
              >
                ...
                <div className="p-8">
                  ...
                  >
                    {loading ? 'Processing...' : isPro ? 'Switch Plan' : 'Upgrade Now'}
                  </motion.button>
